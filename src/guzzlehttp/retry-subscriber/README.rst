...
.. code-block:: javascript
    ...
    {
        "require": {
            "GuzzleHttp5/retry-subscriber": "~1.0"
        }
    }
...
filter
    ...
    ``GuzzleHttp5\Event\AbstractTransferEvent`` object as the second argument.
...
delay
    ...
    ``GuzzleHttp5\Event\AbstractTransferEvent`` as the second argument. The
...
request and a ``GuzzleHttp5\Event\CompleteEvent`` or
``GuzzleHttp5\Event\ErrorEvent`` (both events extend from
``GuzzleHttp5\Event\AbstractTransferEvent``, so you should typehint on that).
...
``GuzzleHttp5\Event\CompleteEvent`` or a ``GuzzleHttp5\Event\ErrorEvent``. The
